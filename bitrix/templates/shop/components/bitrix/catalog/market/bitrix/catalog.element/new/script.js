jQuery(document).ready(function() {
    jQuery('.mycarousel').jcarousel({
       ...
       wrap:"both", //после последнего показывать первый слайд
       auto:0, //время, автоматической смены слайдов
   });
	//Подробнее: http://enkom.kiev.ua/karusel-kontenta-jcarousel.html
	...
	var selectPhoto = function(preview){
		pictureManager.picture = preview;
		if(preview.parent().hasClass("select")) {
			return false;
		}
		var picture = $("#fullPic");
		picture.hide();
		if(preview.hasClass('add-picture')){
			picture.addClass('add-picture');
		}
		else{
			picture.removeClass('add-picture');
		}
		picture.attr("src", preview.attr("href"));
		preview.parent().parent().parent().find('div.select').removeClass("select")
		preview.parent().addClass("select");
		picture.one('load', function() {
			$(this).show();
		});
		return false;
	};
	var previewPictures = $(".addSliderIn ul li a.preview-picture");
	if(previewPictures.length > 1)
	{
		previewPictures.click(function(){return selectPhoto($(this))});
		$(".fullPic-prev").click(function(){prevPhoto()});
		$(".fullPic-next").click(function(){nextPhoto()});
	}
	else
	{
		$(".fullPic-prev").hide();
	...
	var pictureManager = {
		picture: previewPictures.eq(0)
	}
	function prevPhoto(){
		var index = previewPictures.index(pictureManager.picture) - 1;
		if(index < 0) index = previewPictures.length - 1;
		selectPhoto(previewPictures.eq(index));
	}
	function nextPhoto(){
		var index = previewPictures.index(pictureManager.picture) + 1;
		if(index >= previewPictures.length) index = 0;
		selectPhoto(previewPictures.eq(index));
	}
	...
	$("#colours_select span").click(function(){
		...
		selectColor(element);
                $('.colli').removeClass("active");
                $(this).addClass("active");
		return false;
	});
	$(".addSliderTitle .sliderTab:first-child").addClass("active");
	$(".addSliderTitle .sliderTab").click(function(){
		var key = $(this).attr("rel");
		$(".addSliderPages .addSliderB").hide();
		$(".addSliderTitle .sliderTab").removeClass("active");
