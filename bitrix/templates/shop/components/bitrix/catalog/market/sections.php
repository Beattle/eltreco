...
<div class="shopCatalog">
    <div class="shopCatalogL">
		...
		<? 
			...
			{
				$sortField2 = $arParams["ELEMENT_SORT_FIELD"];
				$sortOrder2 = $arParams["ELEMENT_SORT_ORDER"];
				$sortField = "PROPERTYSORT_YML_PRESENCE";
				$sortOrder = "DESC";
			}
		?>
		...
		<?else:?>
			<?if($_REQUEST["set_filter"] == "Y"):?>
				<?
					...
					if($_REQUEST["arrFilter_ff"]["NAME"])
					{
						$arrFilter["NAME"] = "%".$_REQUEST["arrFilter_ff"]["NAME"]."%";
					}
					if($_REQUEST["arrFilter_pf"]["ARTICLE"])
					{
						$arrFilter["PROPERTY_ARTICLE"] = $_REQUEST["arrFilter_pf"]["ARTICLE"];
					}
					if($_REQUEST["arrFilter_pf"]["PAGE_KEYWORDS"])
					{
						$arrFilter["PROPERTY_PAGE_KEYWORDS"] = '%' . $_REQUEST["arrFilter_pf"]["PAGE_KEYWORDS"] . '%';
					}
					if($_REQUEST["arrFilter_ff"]["ID"])
					{
						$arrFilter["ID"] = $_REQUEST["arrFilter_ff"]["ID"];
					}
				?>
			<?else:?>
				<?
					global $arrFilter; 
					$arrFilter = array(
						//"PROPERTY_STATUS_VALUE" => "Распродажа",
						"IBLOCK_TYPE" => "offers",
						"PROPERTY_STATUS" => 13821,
						"IBLOCK_ID" => $_REQUEST['part'] ? IntVal($_REQUEST['part']) : GetOffersIblocks(),
					);
				?>
			<?endif?>
			<?$APPLICATION->IncludeComponent(
...
<?
/* Функционал стал дублировать keywords.txt
if(!$_GET["PAGEN_1"])
{
	...
	if(!$_REQUEST['CUSTOM_TITLE'] && $arProperties["TITLE"])$APPLICATION->SetTitle($arProperties["TITLE"]);
