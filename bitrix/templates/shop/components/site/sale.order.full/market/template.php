...
else
{
	...
	if($arParams["SHOW_MENU"] == "Y")
	{
		...
		<tr>
			...
			<?if ($arResult["CurrentStep"] < 6):?>
				...
				if ($arResult["SKIP_FORTH_STEP"] == "Y")
					unset($arMenuLine[3]);
				if ($arResult["SKIP_THIRD_STEP"] == "Y")
					unset($arMenuLine[2]);
				if ($arResult["SKIP_SECOND_STEP"] == "Y")
					unset($arMenuLine[1]);
				if ($arResult["SKIP_FIRST_STEP"] == "Y")
					unset($arMenuLine[0]);

				$strList = Array();
				foreach ($arMenuLine as $i => $menuLine) {
					if ($arResult["CurrentStep"] > $i + 1)
						$strList[] = "<a href=\"#\" OnClick=\"document.order_form.CurrentStep.value='".($i + 1)."'; document.order_form.BACK.value='Y'; document.order_form.submit(); return false;\">".$arMenuLine[$i]."</a>";
					elseif ($arResult["CurrentStep"] == $i + 1)
						$strList[] = "<b>".$arMenuLine[$i]."</b>";
					else
						$strList[] = $arMenuLine[$i];
				}
				echo implode(" &gt; ", $strList);
				?>
	...
	<tr>
		<td><br />
			<?
				$showStep = $arResult["CurrentStep"];
				if ($arResult["SKIP_FIRST_STEP"] == "Y" && $arResult["CurrentStep"] > 1)
					$showStep--;
				if ($arResult["SKIP_SECOND_STEP"] == "Y" && $arResult["CurrentStep"] > 2)
					$showStep--;
				if ($arResult["SKIP_THIRD_STEP"] == "Y" && $arResult["CurrentStep"] > 3)
					$showStep--;
				if ($arResult["SKIP_FORTH_STEP"] == "Y" && $arResult["CurrentStep"] > 4)
					$showStep--;
			?>
			<? 
			//Add2Log(array($arResult["CurrentStep"], $showStep), false);
			echo ShowError($arResult["ERROR_MESSAGE"]); 

			if ($arResult["CurrentStep"] == 0)
				include($_SERVER["DOCUMENT_ROOT"].$templateFolder."/step0.php");
			elseif ($arResult["CurrentStep"] == 1)
